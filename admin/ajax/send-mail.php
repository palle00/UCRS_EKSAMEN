...
if (!empty($data)) {
    ...
    $select ="SELECT Fest.ID, Fest.Fest_start, Fest.Navn
    FROM Fest
    WHERE Aktiv = 1";
    $result = $link->query($select);
    $row = $result->fetch_assoc();
    $FestID = $row['ID'];
    $Fest_start = $row['Fest_start'];
    $Fest_navn = $row['Navn'];

$select ="SELECT Elev.*
          FROM Elev
          WHERE Kode ='$elev_kode'";
$query_l = $link->query($select);
$row = $query_l->fetch_assoc();
$ElevID = $row['ID'];
...
try {
  ...
  $mail->Username = 'info@implex.dk'; // SMTP username
  $mail->Password = 'Poalle2000'; // SMTP password
  $mail->SMTPSecure = 'tls';
  $mail->Port = 587;

  // Recipients
  $mail->setFrom('info@implex.dk', 'UCRS Fest Billet');
  $mail->addAddress($Email, $navn); // Add a recipient

  // Generate QR code
  // Convert the QR code text to base64
  $qr_code_text = base64_encode($FestID . ',' . $navn . ',' . $Klasse . ',' . 'Drink Billet: ' . $yes);
  ...
  $mail->Body = '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
    ...
    *
    ...
    <tr>
    <td align="center" class="es-m-p0r es-m-p0l" style="Margin:0;padding-top:5px;padding-bottom:5px;padding-left:40px;padding-right:40px"><p style="Margin:0;-webkit-text-size-adjust:none;-ms-text-size-adjust:none;mso-line-height-rule:exactly;font-family:arial, "helvetica neue", helvetica, sans-serif;line-height:21px;color:#ffffff;font-size:14px">' . $Fest_start . '</p></td>"
    </tr>
    <tr>
    ...
    <tr>
    <td align="left" style="padding:0;Margin:0;padding-top:5px"><p style="Margin:0;-webkit-text-size-adjust:none;-ms-text-size-adjust:none;mso-line-height-rule:exactly;font-family:arial, "helvetica neue", helvetica, sans-serif;line-height:21px;color:#ffffff;font-size:14px">' . $Fest_navn . '</p></td>
    </tr>
    </table></td>
    </tr>
    </table></td>
    ...
    <table cellpadding="0" cellspacing="0" width="100%" role="presentation" style="mso-table-lspace:0pt;mso-table-rspace:0pt;border-collapse:collapse;border-spacing:0px">
    <tr>
    <td align="right" style="padding:0;Margin:0"><p style="Margin:0;-webkit-text-size-adjust:none;-ms-text-size-adjust:none;mso-line-height-rule:exactly;font-family:arial, "helvetica neue", helvetica, sans-serif;line-height:21px;color:#ffffff;font-size:14px">' . $pris . 'kr</p></td>
    </tr>
    </table></td>
    </tr>
    </table></td>
